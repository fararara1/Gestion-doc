...
<div class="page-header-custom">
    ...
    <a href="{{ route('meetings.create') }}" class="btn-gold-sm">
            ...
            <tbody>
                @forelse($meetings as $meeting)
                    <tr>
                        ...
                        <td>
                            <span class="badge-gold">{{ $meeting->participants->count() }} participant(s)</span>
                        </td>
                        <td class="text-end">
                            <a href="{{ route('meetings.show', $meeting) }}" class="btn-icon btn-icon-info" title="Consulter">
                                <i class="bi bi-eye"></i>
                            </a>
                            @if(auth()->user()->isAdmin() || auth()->id() === $meeting->user_id)
                                <a href="{{ route('meetings.edit', $meeting) }}" class="btn-icon btn-icon-warning" title="Modifier">
                ...
                @empty
                    <tr>
                        <td colspan="6" class="text-center py-4" style="color: var(--navy-600);">Aucune réunion trouvée.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
